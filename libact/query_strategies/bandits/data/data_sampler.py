...
def sample_albl_data(file_name, context_dim, num_actions, num_contexts,
                       shuffle_rows=True, shuffle_cols=False):
  """Samples bandit problem contextby processing dataframe from combinedFeatures.txt.

  Args:
    file_name: Route of file containing the combinedFeatures dataframe.
    context_dim: Context dimension (i.e. columns with various accuracies).
    num_actions: Number of actions (number of joke ratings to predict).
    num_contexts: Number of contexts to sample.
    shuffle_rows: If True, rows from original dataset are shuffled.
    shuffle_cols: Whether or not context/action jokes are randomly shuffled.
  ...
  combinedFeatures = pickle.load(infile)
  infile.close()
  combinedFeatures = combinedFeatures.iloc[:, :-1]

  if shuffle_cols:
    combinedFeatures = combinedFeatures.iloc[:, np.random.permutation(dataset.shape[1])]
  if shuffle_rows:
    combinedFeatures = combinedFeatures.sample(frac=1)
  dataset = np.array(combinedFeatures.iloc[:num_contexts, :],dtype=float)
