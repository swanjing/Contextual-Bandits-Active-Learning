...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        # still provide the interface to direct access the data by 
        ...
        if self.transform is not None:
            try:
                ...
                xlist = list()
                for pt in x:
                    xlist.append(self.transform(Image.fromarray(pt, mode='L')))
                x = torch.stack(xlist)
    ...
    def get_unlabeled_entries(self):
        ...
        """
        return self._X[~self.get_labeled_mask()], np.where(~self.get_labeled_mask())[0]
...
class DatasetBiome(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            try:
                ...
                xlist = list()
                for pt in x:
                    xlist.append(self.transform(Image.fromarray(pt, mode='RGB')))
                x = torch.stack(xlist)
